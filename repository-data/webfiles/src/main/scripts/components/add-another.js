...
class AddAnother {
    constructor(settings) {
        ...
        this.elementsToHide = [].slice.call(document.querySelectorAll('.form-page:not(.fully-hidden) .form-container__nav, #form-progress, .form-page:not(.fully-hidden) .form-container__title'));
    }
        ...
        if (!this.container.classList.contains('js-event-listeners-added')) {
            this.container.addEventListener('click', event => {
                ...
                if (event.target.classList.contains('js-cancel-item')) {
                    ...
                    const errorSummaryLists = [].slice.call(errorSummaryElement.querySelectorAll('.ds_error-summary__list'));
                    errorSummaryLists.forEach(item => item.parentNode.removeChild(item));
