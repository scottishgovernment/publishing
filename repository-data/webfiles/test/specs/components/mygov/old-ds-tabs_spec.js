...
import Tabs from '../../../../src/main/scripts/components/mygov/old-ds-tabs';
...
describe('tabs', () => {
    ...
    describe('tabs setup', () => {
        it('should clone the data-navigation attribute from the label to the button if present', () => {
            loadFixtures('old-ds-tabs.html');

            // set a data navigation attrib
            document.querySelector('label#tab1-label').dataset.navigation = 'foo';

            testObj.tabs = new Tabs(document.querySelector('.mg_tab-container'));
            testObj.tabs.init();
    ...
    describe('tabs', () => {
        ...
        it('should loop focus back around to the first element when moving past the last tab', () => {
            ...
            expect(document.activeElement === testObj.tabs.tabHeaders[0].querySelector('.mg_tab__label')).toBeTrue();
        });
        ...
        it('should loop focus back around to the last element when moving back from the first tab', () => {
            ...
            expect(document.activeElement === testObj.tabs.tabHeaders[2].querySelector('.mg_tab__label')).toBeTrue();
        ...
        it('should not do anything with tabindex on mobile', () => {
            ...
            // initial state
            const tabIndex1 = testObj.tabs.tabHeaders[0].querySelector('.mg_tab__label').getAttribute('tabindex');
            const tabIndex2 = testObj.tabs.tabHeaders[1].querySelector('.mg_tab__label').getAttribute('tabindex');
            const tabIndex3 = testObj.tabs.tabHeaders[2].querySelector('.mg_tab__label').getAttribute('tabindex');

            // change the selected tab
            testObj.tabs.activateTab(testObj.tabs.tabHeaders[1]);

            expect (testObj.tabs.tabHeaders[0].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual(tabIndex1);
            expect (testObj.tabs.tabHeaders[1].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual(tabIndex2);
            expect(testObj.tabs.tabHeaders[2].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual(tabIndex3);
        });

        it('should set tabindex to -1 on any inactive tabs', () => {

            // initial state
            expect (testObj.tabs.tabHeaders[0].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('0');
            expect (testObj.tabs.tabHeaders[1].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('-1');
            expect (testObj.tabs.tabHeaders[2].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('-1');

            // change the selected tab
            testObj.tabs.activateTab(testObj.tabs.tabHeaders[1]);

            expect (testObj.tabs.tabHeaders[0].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('-1');
            expect (testObj.tabs.tabHeaders[1].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('0');
            expect(testObj.tabs.tabHeaders[2].querySelector('.mg_tab__label').getAttribute('tabindex')).toEqual('-1');
        });
    });
